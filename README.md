...
- **TEST**: New `TestKeywordRoutingBehavior` class in `tests/test_search.py` (3 tests) pins the routing fix: (1) BM25 candidates from other categories survive when no explicit filter is passed, (2) the semantic branch is called with `where=None` when no explicit filter is passed, (3) explicit `category_filter` still overrides the router (regression guard for #109). New `TestPathAwareRanking` class covers the path-metadata boost.
